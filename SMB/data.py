...
def _get_config_file_path(config_name):
    ...
    if not os.path.exists(config_file):
        logging.error("Path does not exist: %s", config_file)
        raise ValueError("Path does not exist:", config_file)
    return config_file


def _write_config(config):
    with open(_get_config_file_path(config['name']), 'w') as json_file:
        logging.info("Writing config: %s", config['name'])
...
def read_config(config_name):
    with open(_get_config_file_path(config_name), 'r') as json_file:
        logging.info("Reading config: %s", config_name)
...
def get_raw_prices_directory():
    ...
    if not os.path.isdir(raw_prices_directory):
        logging.error("Path does not exist: %s", raw_prices_directory)
        raise ValueError("Path does not exist:", raw_prices_directory)
...
def read_prices_file(ticker):
    ...
    logging.info("Reading prices file: %s", file_path)
...
def resample_filter_data(data, resample_interval, exchange_start_time, exchange_end_time):
    try:
        ...
        logging.error("Missing column in data: %s", e)
        return None
    logging.debug(resampled_data.iloc[:10, :])
    logging.debug(resampled_data.isna().sum())
    logging.debug(resampled_data.shape)

    # remove weekends
    hourly_data = resampled_data[resampled_data.index.dayofweek < 5]
    logging.debug(hourly_data.shape)
    ...
    if not isinstance(exchange_start_time, datetime.time) or not isinstance(exchange_end_time, datetime.time):
        logging.error("exchange_start_time and exchange_end_time must be datetime.time instances.")
        raise ValueError("exchange_start_time and exchange_end_time must be datetime.time instances.")
    if exchange_start_time is None:
        logging.error("exchange_start_time must be specified.")
        raise ValueError("exchange_start_time must be specified.")
    if exchange_end_time is None:
        logging.error("exchange_end_time must be specified.")
        raise ValueError("exchange_end_time must be specified.")
    ...
    logging.debug(filtered_data.shape)
...
def _write_model_file(config_name, file_name, targets):
    ...
    logging.info('Writing model file to: %s', file_path)
...
def _read_model_file(config_name, file_name):
    ...
    logging.info('Reading model file: %s', file_path)
